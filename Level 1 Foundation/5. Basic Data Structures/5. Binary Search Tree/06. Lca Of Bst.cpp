...
int lca(node *nd, int d1, int d2)
{
    if(d1 < nd->data  &&  d2<nd->data)
        return lca(nd->left, d1, d2);
    
    else if(d1 > nd->data  &&  d2 > nd->data)
        return lca(nd->right, d1, d2);
    
    else // when they diverge , that means this node is the common ancestor
        return nd->data;
}
