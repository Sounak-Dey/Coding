...
static node *prdcsr, *sccsr;
static int state;

void ps(node *nd, int data)
{
    if(state == 0) // still not found
    {
        if(nd->data == data)
            state = 1;
        else 
    ...
    else if(state == 1) // data found in previous node
    {
        sccsr = nd;
        state = 2;
    }
...
int main()
{
    ...
    if(prdcsr == nullptr)
        ...
        cout<<"Predecessor = "<<prdcsr->data<<endl;
    
    if(sccsr == nullptr)
        cout<<"Successor not found"<<endl;
    else
        cout<<"Successor = "<<sccsr->data<<endl;
