...
void createLeftCloneTree(node *nd) // first go to the end of recursion .. start creating left copies from there
{
    if(nd == nullptr)
        return;

    createLeftCloneTree(nd->left);
    createLeftCloneTree(nd->right);

    node *copyNode = new node(nd->data, nd->left, nullptr); //creating a left copy of 'nd'
    nd->left = copyNode;
}
