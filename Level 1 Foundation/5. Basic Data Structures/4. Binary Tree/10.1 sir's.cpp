...
node *transBackFromLeftClonedTree(node *nd) //using post order
{
    if(nd == nullptr)
        return nullptr;

    node *lnn = transBackFromLeftClonedTree(nd->left->left);
    transBackFromLeftClonedTree(nd->right);

    nd->left = lnn; // removing clones while returning back from recursion
