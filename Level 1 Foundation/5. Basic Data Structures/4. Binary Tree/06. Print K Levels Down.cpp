...
{
    //use queue level order traversal similar

    queue<pair<node*, int>> q;
    int c=0;
    q.push({root, c});

    while(q.front().second < k) //till i reach kth level
    {
        int n = q.size();
        c++;
        for(int i=0; i<n; i++)
        {
            if(q.front().first->left != nullptr) // if left child exists ... push
                q.push({q.front().first->left, c});

            if(q.front().first->right != nullptr)   //if right child exists...push
                q.push({q.front().first->right, c});
