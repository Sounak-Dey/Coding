...
node *removeLeaves(node *nd)
{
    if(nd == nullptr)
        return nullptr;

    if(nd->left == nullptr  &&  nd->right == nullptr) // removing the current node if it is a leaf node
    {
        nd = nullptr;
        return nd;
    }

    node *ln = removeLeaves(nd->left);
    node *rn = removeLeaves(nd->right);
